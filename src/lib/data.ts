...
async function fetchDBInfo(table: string, first: boolean) {
  const { data, error } = await supabase.from(table).select("*");
  
  if (error) {
    console.error("Ошибка при получении personalinfo:", error);
    return {};
  }
  if (first){
    return data?.[0] || {};
...
export const personalInfo = await fetchDBInfo('personalinfo', true);
